	...
	public static partial class SpzUiThemeOps {
		...
		static JArray BuildSurfaces() {
			return new JArray {
				...
				Surface("connection_panels", true, "ConnectionPanel_UI open/reset/IP fields; live status RGB owned by CheckConnection (not BoundChrome)"),
				Surface("top_strip_serv", true, "RestartTheWebui SD SERV / 3D SERV + folder picker (not ConnectionPanel_UI); colors when non-builtin theme"),
