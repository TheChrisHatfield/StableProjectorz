...
- **Interactive docs:** `http://localhost:5557/docs` (Swagger UI)
...
**Installation:** The HTTP server uses FastAPI. Install dependencies:
```bash
pip install -r StreamingAssets/AddonSystem/requirements.txt
```
Or manually: `pip install fastapi uvicorn`

**Usage:** Just make HTTP requests to `http://localhost:5557/api/v1/`
**Interactive API docs:** `http://localhost:5557/docs` (automatically generated by FastAPI)
