//for painting a 2d mask that wraps around a 3d object.
//We drag mouse on the screen, and the object texture gets modified (mask texture gets).
//We are painting a very cheap R8 texture, so fragment function will return 'float', not 'float4'.
//
//NOTICE: there exists 'Object3d_BrushShader_NoConservativeRaster' which you must update too if changing this shader.
...
Shader "Custom/Object3d_BrushShader"{
    ...
    SubShader{
        ...
        Conservative True // removes the seams, but doesn't work on old GPUS like AMD RX560 (GTX 1060 equivalent)
                          // It causes entire shader to malfunction, so we have exact copy of this shader without this setting. 
        Pass{             // 'Object3d_BrushShader_NoConservativeRaster'
            CGPROGRAM
            #pragma target 3.5
            ...
            float _ClickDepth01;      // linear01 depth at the user's click point
            float _DepthFalloffRange;  // depth range for falloff (0 = disabled)
            ...
            float frag (PixelInput i) : SV_Target{
                ...
                // Depth-distance falloff: fade brush to zero for surfaces far from click depth
