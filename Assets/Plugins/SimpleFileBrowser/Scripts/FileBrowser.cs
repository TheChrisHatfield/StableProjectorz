...
#pragma warning restore 0414
        ...
        private void SetGlobalClickBlocker(bool active)
        {
            ...
            if (type == null)
            {
                Debug.LogWarning("Could not find GlobalClickBlocker via reflection.");
                return;
            }

            // 2. Decide which method name we want
            string methodName = active ? "Lock" : "Unlock_if_can";

            // 3. Get the static method
            MethodInfo method = type.GetMethod(methodName, BindingFlags.Public | BindingFlags.Static);

            if (method != null)
            {
                // 4. Invoke it. 
                // First argument is null because it's static.
                // Second argument is the parameters array (passing 'this' as the requester).
