...
public sealed class ForgeNeoSwapPayloadPhaseCTests {
	...
	public void ControlNet_Klein_RejectsAllCnWeights_IncludingFunUnion_AndBypassesDepthGate() {
		...
		Assert.That(hub, Does.Contain("TryHealFamilyMismatchedModels"),
			"Heal must run before Gen Art depth gate so Fun-Union can unlock mismatched leftover CN.");
		...
		Assert.That(list, Does.Contain("Keep explicit None"));
