...
Colors as Pro-Studio Monolith; `control_bg` `#6E7280` stays clearly above `panel_bg` `#1E1F23` so left-column faces remain readable; `corner_radius` 0 (solid-square litmus); `icon_tint` warm; `panel_width` 220; `panel_alpha` 0.92; `font_scale` 0.84; `spacing_scale` 0.94.
